...
class CLMViewSWT
{
    public function setImport_1Toolbar($execute, $upload)
    {
    ...
    public function Import_1(&$spieler, &$lists, &$man, &$swt, &$man_zps, &$vereine, &$sid, $fileName, &$name_manuell)
    {
	...
	<table class="admintable">
		...
		<tr><th width="5%" class="key" nowrap="nowrap"><?php echo JText::_('SWT_TYPE'); ?></th><th class="key" nowrap="nowrap"><?php if ($swt[607] == 255) {
		    echo "Mannschaftsturnier !";
		} else {
		    echo "Einzelturnier ! ";
		} ?></td><tr>
	...
	<div>
        ...
        for ($man = 1; $man < 1 + $swt[603]; $man++) {
			...
			<tr>
				...
				value="<?php if (!$name_manuell[($man - 1)] or $name_manuell[($man - 1)]->name == "") {
				    echo $man_name;
				} else {
				    echo $name_manuell[($man - 1)]->name;
				} ?>" /></td>
			</tr>
			<?php }	?>
			</table>
    ...
    public function setImport_2Toolbar($execute, $upload)
    {
        clm_core::$load->load_css("icons_images");
    ...
    public function Import_2(&$lists, &$swt, &$name_manuell, &$zps_db, &$vereine, &$sid, &$data, &$liga_swt, &$name_man_spl, $fileName, $sg_zps)
    {
	...
	<tr>
		...
		<td width="6%" class="key" nowrap="nowrap"><?php if ($data[$y][6] != "") { ?><img width="16" height="16" src="images/cancel_f2.png" />&nbsp;&nbsp;<?php } else {
		    echo $data[$y][4];
		} ?></td>
		<td width="5%" class="key" nowrap="nowrap" id="mgl<?php echo $data[$y][5]; ?>" name="mgl<?php echo $data[$y][5]; ?>"><?php if ($data[$y][5] == "") { ?><img width="16" height="16" src="images/cancel_f2.png" />&nbsp;<?php } else {
		    echo $data[$y][5];
		}?></td>
		...
		value="<?php if (!$name_man_spl[$data[$y][0]]) {
		    echo $data[$y][2];
		} else {
		    echo $name_man_spl[$data[$y][0]];
		} ?>" /></td>
	</tr>
	<?php
    if ($data[$y + 1][1] > $data[$y][1]) {
        $zps_cnt++;?>
    ...
    public function setImport_3Toolbar($execute, $upload)
    {
        clm_core::$load->load_css("icons_images");
    ...
    public function Import_3(&$lists, &$swt, &$zps_db, &$sid, &$data, &$liga_swt, &$man_zps, $fileName, $sg_zps)
    {
	...
	<tr>
		...
		<td width="50%" nowrap="nowrap">
			...
			<?php /*if(($man_zps[$search->Nr][$z][0] == $sg_zps[1][$search->mnr]) OR ($man_zps[$search->Nr][$z][0] == $sg_zps[2][$search->mnr])) { */?>
				 ...
				 <?php if ($man_zps[$search->Nr][$z][3] == "N") {
				     echo "** ";
				 } echo $man_zps[$search->Nr][$z][0].'-'.$man_zps[$search->Nr][$z][1].'-'.$man_zps[$search->Nr][$z][2]; ?></option> 
				<?php } ?>
			</select>
		</td>
	</tr>
    ...
    public function setSWTToolbar($execute, $upload)
    {
        clm_core::$load->load_css("icons_images");
    ...
    public function SWT(&$rows, &$lists, &$pageNav)
    {
