...
{
    public static function setErgebnisseToolbar($val, $rows, $f_lid, $f_runde, $f_dg)
    {
        // Menubilder laden
        ...
        if ($val == 1) {
            if ($f_lid > 0) {
                $msg = $rows[0]->liga ;
                if ($f_runde > 0) {
                    $msg = $msg.', '.JText::_('TITLE_RESULTS_4').$rows[0]->runde;
                }
                if ($rows[0]->durchgang > 1 and $f_dg > 0) {
                    $msg = $msg.', '.JText::_('TITLE_RESULTS_5').$rows[0]->dg;
                }
            } else {
                if ($f_runde > 0) {
                    $msg = JText::_('TITLE_RESULTS_6').$rows[0]->runde;
                } else {
                    $msg = JText::_('TITLE_RESULTS_7');
                }
            }
            JToolBarHelper::title(JText::_('TITLE_RESULTS_2').$msg.JText::_('TITLE_RESULTS_3'), 'clm_settings.png');
        } else {
            JToolBarHelper::title(JText::_('TITLE_RESULTS_1'), 'generic.png');
        }
        ...
        if ($val == 1) {
            JToolBarHelper::custom('back', 'cancel.png', 'download_f2.png', JText::_('MEMBER_BUTTON_BACK'), false);
        }
        JToolBarHelper::help('screen.clm.ergebnisse');
    }

    public static function ergebnisse($rows, $lists, $pageNav, $option)
    {
        ...
        for ($i = 0, $n = count($rows); $i < $n; $i++) {
			...
			<tr class="<?php echo 'row'. $k; ?>">
				...
				<td align="center">
					...
					<td align="center">
					<?php if ($row->gemeldet > 0) { ?><img width="16" height="16" src="components/com_clm/images/apply_f2.png" /> <?php } else { ?><img width="16" height="16" src="components/com_clm/images/cancel_f2.png" /> <?php }
					if ($row->dwz_editor > 0) { ?> 
					<img width="16" height="16" src="components/com_clm/images/extensions_f2.png" /><?php }
					?>
				</td>
				<td align="center">
					<?php if ($row->gemeldet == 1) {
					    echo JText::_('RESULTS_OVERVIEW_FREE');
					} else {
					    echo $rows[$i]->uname;
					} ?>
				</td>
    ...
    public static function setErgebnisToolbar($runde)
    {
    ...
    public static function Ergebnis($row, $runde, $heim, $hcount, $gast, $gcount, $bretter, $ergebnis, $option, $hvoraufstellung, $gvoraufstellung)
    {
			 ...
			 <option value="<?php echo $heim[$x]->mgl_nr.'-'.$heim[$x]->zps; ?>"
			 	<?php if (($bretter and ((int)$heim[$x]->mgl_nr) == ((int)$bretter[$i]->spieler) and $heim[$x]->zps == $bretter[$i]->zps)
			            // Bedingungen Voreinstellung
			                ...
			                or (!$bretter and isset($hvoraufstellung[$i]) and ($hvoraufstellung[$i]->snr - 1) == $x and isset($bretter[$i]) and $bretter[$i]->zps != "ZZZZZ")) { ?> selected="selected" <?php } ?>>
			...
			<option value="<?php echo $gast[$x]->mgl_nr.'-'.$gast[$x]->zps; ?>" 
				 <?php if (($bretter and ((int)$gast[$x]->mgl_nr) == ((int)$bretter[$i]->gegner) and $gast[$x]->zps == $bretter[$i]->gzps)
			             // Bedingungen Voreinstellung
			                ...
			                or (!$bretter and isset($gvoraufstellung[$i]) and ($gvoraufstellung[$i]->snr - 1) == $x and isset($bretter[$i]) and $bretter[$i]->gzps != "ZZZZZ")) { ?> selected="selected" <?php } ?>>
...
<!--			<select name="ko_decision" id="ko_decision" value="<?php echo $runde[0]->ko_decision; ?>" class="js-example-basic-single" size="1"> -->
			...
			<option value="1" <?php if ($runde[0]->ko_decision == 1) {
			    echo 'selected="selected"';
			} ?>><?php echo JText::_('RESULTS_MT_KO_DECISION_BW');?></option>
			<option value="2" <?php if ($runde[0]->ko_decision == 2) {
			    echo 'selected="selected"';
			} ?>><?php echo JText::_('RESULTS_MT_KO_DECISION_BLITZ').$runde[0]->hname;?></option>
			<option value="3" <?php if ($runde[0]->ko_decision == 3) {
			    echo 'selected="selected"';
			} ?>><?php echo JText::_('RESULTS_MT_KO_DECISION_BLITZ').$runde[0]->gname;?></option>
			<option value="4" <?php if ($runde[0]->ko_decision == 4) {
			    echo 'selected="selected"';
			} ?>><?php echo JText::_('RESULTS_MT_KO_DECISION_LOS').$runde[0]->hname;?></option>
			<option value="5" <?php if ($runde[0]->ko_decision == 5) {
			    echo 'selected="selected"';
			} ?>><?php echo JText::_('RESULTS_MT_KO_DECISION_LOS').$runde[0]->gname;?></option>
			</select>
			</td>
		</tr>
	</table>
...
<?php // }?>	
		...
		<tr>
			...
			<td>
			<?php if ($runde[0]->dwz_zeit != "0000-00-00 00:00:00" and $runde[0]->dwz_zeit != "1970-01-01 00:00:00") {
...
<?php 	for ($i = 0; $i < $runde[0]->stamm; $i++) { ?>
	<tr>
		...
		<td class="key" nowrap="nowrap">
			<?php for ($x = 0; $x < 11; $x++) { ?>
			<?php if ($runde[0]->gemeldet > 0 and (((int)$ergebnis[$x]->id) - 1) == ((int)$bretter[$i]->ergebnis)) {
