...
function clm_api_db_trf_import($file, $season, $turnier, $group = false, $update = false, $test = false)
{
    ...
    if ($test) {
        echo "<br><br>Test - keine Übernahme der Daten ins CLM!";
    }
    $new_ID = 0;
    if ($debug > 0) {
    ...
    if ($debug > 0) {
        echo "<br><br>datei: ".$file;
    } 		//echo "<br>end"; //die();
    if ($debug > 0) {
        echo "<br>saison: ".$season;
    } 	//echo "<br>end"; //die();
    if ($debug > 0) {
        echo "<br>turnier: ".$turnier;
    } 	//echo "<br>end"; //die();
    ...
    foreach ($lines as $line_num => $line) {
        ...
        if ($din == '001') {
            $a_spielerdaten[] = str_replace("’", "'", $line);
            continue;
        }
        if ($din == '013') {
            $a_teamdaten[] = str_replace("’", "'", $line);
            continue;
        }
        if ($din == '132') {
            $rundendata = substr($line, 91);
            continue;
        }
        if ($din == 'XCT') {
            $rundentime = substr($line, 91);
            continue;
        }
        ...
        switch ($din) {
            ...
            case 'XXS': $a_xxs = explode(' ', $value);
                ...
                break;
            case 'XCA': $xca_rounds = $value;
                break;
            case 'XCZ': $snr = trim(substr($line, 4, 4));
                $a_spielerCZ[$snr] = str_replace("’", "'", $line);
                break;
            case 'XCC': $snr = trim(substr($line, 4, 4));
                $a_spielerCC[$snr] = str_replace("’", "'", $line);
                break;
            case 'XCR': $snr = trim(substr($line, 4, 4));
                $a_spielerCR[$snr] = str_replace("’", "'", $line);
                break;
        }
    }
    if (isset($xxs_pab) and ($xxs_pab == '0.5' or $xxs_pab == '.5')) {
        $erg_pab = 12;
    } elseif (isset($xxs_pab) and $xxs_pab == 1) {
        $erg_pab = 5;
    } else {
        $erg_pab = 13;
    }
    if ($debug > 0) {
        echo "<br>xxs_pab: $xxs_pab   erg: $erg_pab ";
    }
    if (!isset($rundendata)) {
        $rundendata = '';
    }
    ...
    if ($group) { 	// Mannschaftsturniere
        ...
        if (strpos($file, '.TUT') > 0 or strpos($file, '.tut') > 0) {
            $typ = '2';
        } // Mannschaft-Rundenturnier	 .TUT
        ...
        if (count($a_teamdaten) > 0) {
            $params_array[] = 'teamranking=4';
        }
        if ($debug > 2) {
            echo "<br>params_array: ";
    ...
    if ($debug > 0) {
        echo "<br>neue Turnier-ID (trf): ";
        var_dump($new_swt_tid);
    }
    //die('Turnierdaten');

    //	Rundendaten -> Tabelle clm_turniere_rnd_termine
    if ($debug > 0) {
        echo "<br><br>-- Rundendaten --";
    }
    //		$rundendata = substr($rundendata,87);
    if ($debug > 0) {
        echo "<br>Rundendaten: $rundendata ";
    }
    ...
    foreach ($a_spielerdaten as $line) {
        ...
        if ($debug > 0) {
            echo "<br>snr: $snr  geschlecht: $geschlecht  titel: $titel  name: $name  elo: $FIDEelo  cco: $FIDEcco  id: $FIDEid  birthyear: $birthYear  points: $points  rang: $rang";
        }
        $a_name[$snr] = $name;
        ...
        if (isset($a_spielerCR[$snr])) {
            ...
            if (is_numeric($start_dwz)) {
                $start_dwz = (int) $start_dwz;
            } else {
                $start_dwz = 0;
            }
        ...
        if ($debug > 0) {
            echo "<br>snr: $snr  XCZ zps: $zps  mgl_nr: $mgl_nr  XCC verein $verein  XCR start_dwz: $start_dwz  FIDEelo: $FIDEelo twz: $twz ";
        }
        if ($group) { 	// Mannschaftsturniere
        } else { 		// Einzelturniere
    ...
    if ($debug > 0) {
        echo "<br><br>-- Einzel-Ergebnisdaten --";
    }
    ...
    foreach ($a_spielerdaten as $line) {
        ...
        for ($i = 0; $i < $rundenzahl; $i++) {
            ...
            if ($debug > 0) {
                echo "<br> ( Runde: ".$runde."  Spieler: $spieler  Gegner: $gegner  Color: $color  Result: $result ) ";
            }
    ...
    for ($i = 1; $i <= $rundenzahl; $i++) {
        for ($j = 1; $j <= $a_brett[$i]; $j++) {
            if ($debug > 1) {
                ...
                var_dump($a_paarung[$i][$j]);
            }
            if ($debug > 0) {
                echo "<br>Runde: ".$i."  Brett: $j   ".$a_name[$a_paarung[$i][$j]['spieler']]."  ".$a_paarung[$i][$j]['werg'];
