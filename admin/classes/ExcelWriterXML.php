...
@param string $function The name of the function that was called
     ...
     */
    public function docTitle($title = '')
    {
        $this->docTitle = $title;
    }
     ...
     */
    public function docSubject($subject = '')
    {
     ...
     */
    public function docAuthor($author = '')
    {
     ...
     */
    public function docManager($manager = '')
    {
     ...
     */
    public function docCompany($company = '')
    {
        $this->docCompany = $company;
    }
...
function EWXmysqlGenerateByQuery(&$sheet, $link, $query)
{
    ...
    if (!$res) {
        $sheet->addError('Database', 'Unable to execute query ('.mysql_error().')');
    }
    if (count($sheet->formatErrors) > 0) {
        return false;
    }
