...
if (isset($saisonpunkte[0])) {
    $ligapunkte = $saisonpunkte[0];
}

if ($saison[0]->dsb_datum  > 0) {
...
if (($saison[0]->dsb_datum == 0) || (!isset($saison))) {
...
$w = 0;
	...
	<tr class="<?php echo $zeilenr; ?>">
		...
		<td class="punkte clmborder"><?php if (isset($brett[$x]->count)) {
		    echo $brett[$x]->count;
		} ?></td>
		<td class="punkte"><?php if (isset($brett[$x]->sum)) {
		    echo str_replace('.0', '', $brett[$x]->sum);
		} ?></td>
<!--		<td class="punkte"><?php if (isset($brett[$x]->sum)) {
    echo round((($brett[$x]->sum * 100) / ($brett[$x]->count)), 1); ?></td>-->
		<td class="punkte"><?php if (isset($brett[$x]->sum)) {
		    echo round(100 * ($brett[$x]->sum - $brett[$x]->count * $ligapunkte->antritt) / ($brett[$x]->count * $ligapunkte->sieg), 1);
		} ?></td>
		
<!--		<td class="punkte"><?php echo $brett[$x]->count - $brett[$x]->sum;
} ?></td>-->
		<td class="punkte"><?php if (isset($brett[$x]->sum)) {
		    echo str_replace('.0', '', $brett[$x]->count * ($ligapunkte->sieg + $ligapunkte->antritt) - $brett[$x]->sum);
		} ?></td>
		
<!--		<td class="punkte"><?php if (isset($brett[$x]->sum)) {
    echo 100 - round((($brett[$x]->sum * 100) / ($brett[$x]->count)), 1);
} ?></td>-->
		<td class="punkte"><?php if (isset($brett[$x]->sum)) {
		    echo 100 - round(100 * ($brett[$x]->sum - $brett[$x]->count * $ligapunkte->antritt) / ($brett[$x]->count * $ligapunkte->sieg), 1);
		} ?></td>
		
		
<?php if (isset($wbrett[$w]->brett) and $wbrett[$w]->brett == $x + 1) {
    if (!isset($sbrett[$w])) {
        $sbrett[$w] = new StdClass();
        $sbrett[$w]->sum = 0;
    }
    if ($x % 2 != 0) { ?>
		<td class="white clmborder"><?php echo $sbrett[$w]->sum; ?></td>
		<td class="white"><?php echo round((($sbrett[$w]->sum * 100) / $brett[$x]->count), 1); ?></td>
		<?php $sum_weiss = $sum_weiss + $sbrett[$w]->sum;
    } else { ?>
...
<br>
<?php $count = count($spieler);
    $ex = 0; ?>
<h4><?php echo JText::_('SEASON_RATING_BEST_PLAYER_I') ?> <?php if ($count < 10 and $count > 0) {
    echo $count;
} else { ?> 10<?php } ?> <?php echo JText::_('SEASON_RATING_BEST_PLAYER_II') ?></h4>
<?php if (!$spieler) { ?>
...
if ($count < 10) {
	...
	<tr class="<?php echo $zeilenr; ?>">
		...
		<td class="name" align="left"><a href="index.php?option=com_clm&view=spieler&saison=<?php echo $sid; ?>&zps=<?php echo $spieler[$x]->zps; ?>&PKZ=<?php echo $spieler[$x]->PKZ; ?>&mglnr=<?php echo $spieler[$x]->mgl_nr; ?><?php if ($itemid <> '') {
		    echo "&Itemid=".$itemid;
		} ?>"><?php echo $spieler[$x]->Spielername; ?></a></td>
		<td class="dwz" align="center"><?php echo $spieler[$x]->DWZ; ?></td>
		<td class="verein" align="left"><a href="index.php?option=com_clm&view=verein&saison=<?php echo $sid; ?>&zps=<?php echo $spieler[$x]->zps; ?><?php if ($itemid <> '') {
		    echo "&Itemid=".$itemid;
		} ?>"><?php echo $spieler[$x]->Vereinname; ?></a></td>
		<td class="punkte" align="center"><?php echo $spieler[$x]->Punkte; ?></td>
		<td class="niveau"><?php echo $spieler[$x]->Niveau; ?></td>
		<td class="leistung"><?php if ($spieler[$x]->Punkte == $spieler[$x]->Partien and $spieler[$x]->Leistung > 0) {
		    echo (($spieler[$x]->Leistung)).' &sup2';
		    $ex = 1;
		} else {
		    echo $spieler[$x]->Leistung;
		} ?></td>
...
<br>
<h4><?php echo JText::_('SEASON_RATING_BEST_TEAM_I') ?> <?php if (count($mannschaft) < 5 and count($mannschaft) > 0) {
    echo count($mannschaft);
    $counter = count($mannschaft);
} else { ?>5<?php $counter = 5 ;
} ?> <?php echo JText::_('SEASON_RATING_BEST_TEAM_II') ?></h4>
...
<?php for ($x = 0; $x < $counter; $x++) {
	...
	<tr class="<?php echo $zeilenr; ?>">
		<td class="nr" align="center"><?php echo $x + 1; ?></td>
		<td class="mannschaft" align="left"><a href="index.php?option=com_clm&view=mannschaft&saison=<?php echo $sid; ?>&liga=<?php echo $mannschaft[$x]->lid; ?>&tlnr=<?php echo $mannschaft[$x]->tln_nr; ?><?php if ($itemid <> '') {
		    echo "&Itemid=".$itemid;
		} ?>"><?php echo $mannschaft[$x]->name; ?></a></td>
		<td class="liga" align="left"><a href="index.php?option=com_clm&view=rangliste&saison=<?php echo $sid; ?>&liga=<?php echo $mannschaft[$x]->lid; ?><?php if ($itemid <> '') {
		    echo "&Itemid=".$itemid;
