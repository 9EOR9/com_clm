...
<div id="termine">
    ...
    <?php  } else {  ?>
    	<table>
        	...
        	<tr>
            	...
            	<td><a href="index.php?option=com_clm&view=verein&saison=<?php echo $sid; ?>&zps=<?php echo $termine_detail[0]->host;
				    if ($itemid <> '') {
				        echo "&Itemid=". $itemid;
				    } ?>"><?php echo $termine_detail[0]->hostname; ?></a></td>
        		<?php } else { ?>
        ...
        if ($termine_detail[0]->starttime != '00:00:00') {
            echo '&nbsp;&nbsp;'.substr($termine_detail[0]->starttime, 0, 5).'&nbsp;Uhr';
        }
        if ($termine_detail[0]->allday == 1) {
            echo '&nbsp;&nbsp;'.'&nbsp;ganzt&auml;gig';
        }
        if ($termine_detail[0]->enddate != 0) {
            if (($termine_detail[0]->enddate != $termine_detail[0]->startdate) or ($termine_detail[0]->endtime != '00:00:00') or ($termine_detail[0]->noendtime == 1)) {
                echo "&nbsp;-&nbsp;";
            }
            if ($termine_detail[0]->enddate != $termine_detail[0]->startdate) {
                echo $arrWochentag[date("l", $enddate[0])]. ",&nbsp;". JHTML::_('date', $termine_detail[0]->enddate, JText::_('DATE_FORMAT_CLM')).'&nbsp;&nbsp;';
            }
            if ($termine_detail[0]->endtime != '00:00:00') {
                if ($termine_detail[0]->noendtime == 1) {
                    echo 'ca.&nbsp;';
                }
    ...
    <a href="index.php?option=com_clm&amp;view=termine&amp;categoryid=<?php echo $categoryid; ?>&amp;saison=<?php echo $sid;
if ($itemid <> '') {
    echo "&Itemid=". $itemid;
} ?>"><?php echo JText::_('TERMINE_BACK') ?></a>
	<br>
