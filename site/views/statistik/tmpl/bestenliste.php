...
<?php } else {
    // Sortierung
    if ($itemid <> '') {
        $plink = '&saison=' . $sid . '&liga='. $lid .'&layout=bestenliste&Itemid='.$itemid;
    } else {
        $plink = '&saison=' . $sid . '&liga='. $lid .'&layout=bestenliste' ;
    } ?>
		...
		<?php for ($xx = 1; $xx < 7; $xx++) {   //max. 6 Spalten
		    ...
		    elseif ($params['btiebr'.$xx] == 2) {
		        ...
		        if (($bestenliste[$x]->Punkte == $bestenliste[$x]->Partien) and ($bestenliste[$x]->Leistung > 0)) {
		            $hstring = $bestenliste[$x]->Leistung.' &sup2;';
		            $ex = 1;
		        } else {
