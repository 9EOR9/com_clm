...
for ($x = 0; $x < ($liga[0]->stamm + 1); $x++) {
    ...
    for ($xx = 1; $xx < 7; $xx++) {   //max. 6 Spalten
        $en = 0;
        if ($params['btiebr'.$xx] == 1) {
            $hstring = JText::_('LEAGUE_STAT_PLAYERPOINTS');
            $width = 9;
        } elseif ($params['btiebr'.$xx] == 2) {
            $hstring = JText::_('LEAGUE_STAT_PLAYERGAMES');
            $width = 9;
        } elseif ($params['btiebr'.$xx] == 3) {
            $hstring = JText::_('LEAGUE_STAT_PLAYERLEVEL');
            $width = 12;
        } elseif ($params['btiebr'.$xx] == 4) {
            $hstring = JText::_('LEAGUE_STAT_RATING_PDF');
            $en = 1;
            $width = 13;
        } elseif ($params['btiebr'.$xx] == 5) {
            $hstring = JText::_('LEAGUE_STAT_PERCENT');
            $width = 9;
        } elseif ($params['btiebr'.$xx] == 6) {
            $hstring = JText::_('LEAGUE_STAT_PLAYERPOINTS');
            $width = 11;
            $en = 3;
            $ey = 1;
        } elseif ($params['btiebr'.$xx] == 7) {
            $hstring = JText::_('LEAGUE_STAT_PLAYERGAMES');
            $width = 11;
            $en = 3;
            $ey = 1;
        } elseif ($params['btiebr'.$xx] == 8) {
            $hstring = JText::_('LEAGUE_STAT_PERCENT');
            $width = 9;
            $en = 3;
            $ey = 1;
        } elseif ($params['btiebr'.$xx] == 9) {
            $hstring = JText::_('LEAGUE_STAT_BNUMBERS_PDF');
            $width = 13;
        }
        if ($params['btiebr'.$xx] > 0) {
            if ($en == 1) {
                $pdf->Cell($width, $zelle, clm_core::$load->utf8decode($hstring).clm_core::$load->utf8decode(html_entity_decode('&sup1;', ENT_COMPAT)), 1, 0, 'C', 1);
            } elseif ($en == 3) {
                $pdf->Cell($width, $zelle, clm_core::$load->utf8decode($hstring).clm_core::$load->utf8decode(html_entity_decode('&sup3;', ENT_COMPAT)), 1, 0, 'C', 1);
            } else {
                $pdf->Cell($width, $zelle, clm_core::$load->utf8decode($hstring), 1, 0, 'C', 1);
            }
        }
    }
            ...
            for ($xx = 1; $xx < 7; $xx++) {   //max. 6 Spalten
                ...
                elseif ($params['btiebr'.$xx] == 2) {
                    ...
                    if (($spielerbrett->Punkte == $spielerbrett->Partien) and ($spielerbrett->Leistung > 0)) {
                        $hstring = ($spielerbrett->Niveau) + 667;
                        $en = 2;
                        $ex = 1;
                    } else {
                ...
                if ($params['btiebr'.$xx] > 0) {
                    if ($en == 2) {
                        $pdf->Cell($width, $zelle, $hstring.clm_core::$load->utf8decode(html_entity_decode('&sup2;', ENT_COMPAT)), 1, 0, 'C', 1);
                    } else {
                        $pdf->Cell($width, $zelle, $hstring, 1, 0, 'C', 1);
                    }
                }
            }
            $pdf->Cell(1, $zelle, '', 0, 1, 'C');
