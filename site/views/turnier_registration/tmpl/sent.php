...
if (!is_numeric($reg_elo)) {
    $reg_elo = 0;
} else {
    $reg_elo = (int) $reg_elo;
}
if (!is_numeric($reg_dwz)) {
    $reg_dwz = 0;
} else {
    $reg_dwz = (int) $reg_dwz;
}
if (!is_numeric($reg_mgl_nr)) {
    $reg_mgl_nr = 0;
} else {
    $reg_mgl_nr = (int) $reg_mgl_nr;
}
...
if ($reg_vorname != '') {
    $body_daten .=  JText::_('REGISTRATION_VORNAME').': '.$reg_vorname."\n";
}
$body_daten .=	JText::_('REGISTRATION_JAHR').': '.$reg_birthYear."\n"
            . JText::_('REGISTRATION_CLUB').': '.$reg_club."\n"
            . JText::_('REGISTRATION_MAIL').': '.$reg_mail."\n";
if ($reg_dwz != '') {
    $body_daten .=  JText::_('REGISTRATION_DWZ').': '.$reg_dwz."\n";
}
if ($reg_elo != '') {
    $body_daten .=  JText::_('REGISTRATION_ELO').': '.$reg_elo."\n";
}
if ($reg_FIDEid != '') {
    $body_daten .=  JText::_('REGISTRATION_FIDEID').': '.$reg_FIDEid."\n";
}
if ($reg_tel_no != '') {
    $body_daten .=  JText::_('REGISTRATION_TEL_NO').': '.$reg_tel_no."\n";
}
if ($reg_account != '') {
    $body_daten .=  JText::_('REGISTRATION_ACCOUNT_'.$typeAccount).': '.$reg_account." \n\n";
}
if ($reg_comment != '') {
    $body_daten .=  JText::_('REGISTRATION_COMMENT').': '."\n".$reg_comment."\n";
}
