...
$auser = (int) clm_core::$access->getId(); // aktueller CLM-User
...
// Sortierung
if ($itemid <> '') {
    $plink = '&saison=' . $sid . '&Itemid='.$itemid;
} else {
    $plink = '&saison=' . $sid ;
}
                ...
                <form name="form1">
                    <select name="select" onchange="goto(this.form)" class="selectteam">
						<?php foreach ($saisons as $saisons) { ?>
                            <option value="<?php echo JURI::base(); ?>index.php?option=com_clm&view=vereinsliste&saison=<?php echo $saisons->id; ?><?php if ($itemid <> '') {
                                echo "&Itemid=".$itemid;
                            } ?>"
...
<table cellpadding="0" cellspacing="0" class="vereinsliste">
 ...
 <!--       <th rowspan="2" class="col_1"><a href="javascript:tableOrdering('name','asc','');"><?php echo JText::_('CLUBS_LIST_NAME') ?></a></th> -->
    ...
    <tr>
		...
		<td class="col_1"><a href="index.php?option=com_clm&view=verein&saison=<?php echo $sid; ?>&zps=<?php echo $vereine[$z]->zps; ?><?php if ($itemid <> '') {
		    echo "&Itemid=".$itemid;
		} ?>"><?php echo $vereine[$z]->name; ?></a></td>
		<?php } else { ?>	
       ...
       <?php if ($fe_vereinsliste_vs == 1) { ?><td class="col_2"><?php // Wenn Email
        ...
        <td class="col_4"><a href="index.php?option=com_clm&view=dwz&saison=<?php echo $sid; ?>&zps=<?php echo $vereine[$z]->zps; ?><?php if ($itemid <> '') {
            echo "&Itemid=".$itemid;
        } ?>"><?php echo $vereine[$z]->MGL_SUM; ?></a></td>
        ...
        <?php if ($fe_vereinsliste_elo == 1) { ?>
        <td class="col_8">
        <?php if ($vereine[$z]->FIDE_Elo == 0) {
            echo "-";
        } else {
            echo round($vereine[$z]->FIDE_Elo) . '(' . $vereine[$z]->ELO_SUM .')' ;
        } ?>
        </td>
		<?php } ?>
    </tr>
